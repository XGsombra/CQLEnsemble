...
class CQLSAC(nn.Module):
    ...
    def __init__(self,
        ...
        # self.target_entropy = -np.prod(env.action_space.shape[0]).item() #-action_size  # -dim(A)
    ...
    def learn(self, experiences):
        ...
        cql1_scaled_loss = ((torch.logsumexp(cat_q1 / self.temp,
                                             dim=1).mean() * self.temp) - q1.mean()) * self.cql_weight
        cql2_scaled_loss = ((torch.logsumexp(cat_q2 / self.temp,
                                             dim=1).mean() * self.temp) - q2.mean()) * self.cql_weight
