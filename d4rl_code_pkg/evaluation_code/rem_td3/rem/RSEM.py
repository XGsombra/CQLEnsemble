...
class Critic(nn.Module):
	def __init__(self, state_dim, action_dim, num_heads):
		...
		self.l6 = nn.Linear(300, num_heads)
...
class RSEM(object):
	...
	def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2, noise_clip=0.5, policy_freq=2):
		...
		for it in range(iterations):
			...
			target_Q1 = torch.matmul(target_Q1, alpha1)
			target_Q2 = torch.matmul(target_Q2, alpha2)
			target_Q = torch.min(target_Q1, target_Q2)
			target_Q = reward + (done * discount * target_Q).detach()

			# Get current Q estimates
			current_Q1, current_Q2 = self.critic(state, action)
			current_Q1 = torch.matmul(current_Q1, alpha1)
			current_Q2 = torch.matmul(current_Q2, alpha2)
			# Compute critic loss
