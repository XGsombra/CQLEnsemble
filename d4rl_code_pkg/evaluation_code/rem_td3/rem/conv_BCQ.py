...
class Actor(nn.Module):
    ...
    def forward(self, state, action):
        state = torch.reshape(state, (-1,48,48,3))
        ...
        a = F.relu(self.l1(torch.cat([hidden, action], 1)))
        #a = F.relu(self.l2(a))
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
            ...
            self.conv1a = nn.Conv2d(3, 5, 5)  # in ch, out ch, kernel
            self.pool1a = nn.MaxPool2d(2)
            self.conv2a = nn.Conv2d(5, 5, 5)
            self.pool2a = nn.MaxPool2d(2)
            self.hidden_dim = 5*9*9
            self.l1a = nn.Linear(self.hidden_dim + action_dim, 32)
            self.l3a = nn.Linear(32, action_dim)
            #self.l1 = nn.Linear(state_dim + action_dim, 400)
            #self.l2 = nn.Linear(400, 300)
            #self.l3 = nn.Linear(300, 1)

            self.conv1b = nn.Conv2d(3, 5, 5)  # in ch, out ch, kernel
            self.pool1b = nn.MaxPool2d(2)
            self.conv2b = nn.Conv2d(5, 5, 5)
            self.pool2b = nn.MaxPool2d(2)
            self.hidden_dim = 5*9*9
            self.l1b = nn.Linear(self.hidden_dim + action_dim, 32)
            self.l3b = nn.Linear(32, action_dim)
    ...
    def forward(self, state, action):
            ...
            hidden = F.relu(self.conv1a(state))
            hidden = self.pool1a(hidden)
            hidden = F.relu(self.conv2a(hidden))
            hidden = self.pool2a(hidden)
            x = hidden = torch.reshape(hidden, (-1, self.hidden_dim))
            xu = torch.cat([x, action], 1)
            x1 = F.relu(self.l1a(xu))
            q1 = self.l3a(x1)

            hidden = F.relu(self.conv1b(state))
            hidden = self.pool1b(hidden)
            hidden = F.relu(self.conv2b(hidden))
            hidden = self.pool2b(hidden)
            x = hidden = torch.reshape(hidden, (-1, self.hidden_dim))
            xu = torch.cat([x, action], 1)
            x1 = F.relu(self.l1b(xu))
            q2 = self.l3b(x1)
            return q1, q2
...
class VAE(nn.Module):
    def __init__(self, state_dim, action_dim, latent_dim, max_action):
            ...
            self.e1 = nn.Linear(self.hidden_dim + action_dim, 128)
            ...
            self.d1 = nn.Linear(self.hidden_dim + latent_dim, 128)
            self.d2 = nn.Linear(128, 128)
            self.d3 = nn.Linear(128, action_dim)
    ...
    def forward(self, state, action):
            ...
            hidden = F.relu(self.conv2(hidden))
            hidden = self.pool2(hidden)
            hidden = torch.reshape(hidden, (-1, self.hidden_dim))

            z = F.relu(self.e1(torch.cat([hidden, action], 1)))
            z = F.relu(self.e2(z))
    ...
    def decode(self, state, z=None):
            ...
            a = F.relu(self.d1(torch.cat([hidden, z], 1)))
            a = F.relu(self.d2(a))
